...
> **2026-09-09 自动清理更新**：报告保存与核验结束后，自动清理当天下载 PDF、去重副本和解析缓存；并发任务结束后统一清理，失败任务同样回收残留。保留报告/财务 Raw/底稿/模型缓存。全量 338 项测试通过，详见 README 的“报告完成后自动清理下载文件”。

> **2026-09-09 当前版本更新：V1.3**。报告/规则版本 1.3，60 项检查，新增 8 项深度检查、重叠评分合并与原始响应/审核底稿追溯。用户已授权升级原 52 项基线；下文涉及 V1.2、规则 1.1、固定 52 项及原测试数字的内容为历史记录。当前行为和限制以 [V1.3 升级说明](docs/V1.3_RULE_UPGRADE.md) 为准。
