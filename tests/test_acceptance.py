...
由原始验收测试迁移；修正契约相关的构造输入，保留原始验收文件供追溯。
"""
import asyncio
import copy
...
class AuditChecks(unittest.TestCase):
    ...
    def test_29_llm_budget_partial_failure_visible(self):
        llm=LLMAdapter(LLMConfig(api_key="fixture",budget_cny=.35,max_output_tokens=1000,price_out_cny_per_1m=300))
        def chat(*a,**kw):
            llm.spent_cny+=.1
    ...
    def test_33_ai_downgrade_recomputes_coverage(self):
        ...
        next(o for o in out.outcomes if o.rule.rule_id=="FQ08").evidence_ids=["d:p1"]
    ...
    def test_41_completed_task_not_partial_for_basis_note(self):
        ...
        vals += [fact("revenue", f"{year}-12-31", 80., PeriodType.ANNUAL) for year in range(2021,2026)]
        ...
        self.assertEqual(result.payload["gaps"],[])
        self.assertTrue(any(note.startswith("计算基准") for note in result.payload["notes"]))
    ...
    def test_49_adjusted_debt_ratio_denominator(self):
        ...
        vals=[fact(k,"2026-06-30",v) for k,v in
              {"total_assets":100.,"total_liabilities":80.,"advance_receivables":20.,"contract_liabilities":0.}.items()]
